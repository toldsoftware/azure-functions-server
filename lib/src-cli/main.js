...
var isProduction = process.argv.some(function (x) { return x.match(/--prod/) != null; });
var isClient = process.argv.some(function (x) { return x.match(/--client/) != null; });
var shouldInject = process.argv.some(function (x) { return x.match(/--debug/) != null; });
var iSrcPathName = process.argv.map(function (x, i) { if (x.match(/--src/) != null) {
    return i;
} }).filter(function (x) { return x != null; })[0];
var srcPathName = iSrcPathName != null ? process.argv[iSrcPathName + 1] : 'lib';
                                ...
                                return tslib_1.__generator(this, function (_a) {
                                    switch (_a.label) {
                                        ...
                                        return [4 /*yield*/, new Promise(function (resolve, reject) {
                                                ...
                                                try {
                                                    ...
                                                    var functions = functionNames
                                                        .map(function (x) { return "{name: '" + x + "', main: require('" + ("./../../" + srcPathName + "/src-server/" + x) + "').main }"; })
                                                        .join(',\n\t');
                                                    console.log('Create Test Main');
                                        ...
                                        case 1:
                                            // Create Test Main
                                            _a.sent();
                                            if (!(isProduction || isClient)) return [3 /*break*/, 4];
                                            // Wabpack
                                            console.log('Webpack');
                                            return [4 /*yield*/, run_webpack_1.runWebpackClient(entrySourceFiles_1, shouldInject)];
                                        case 2:
                                            _a.sent();
                                            if (!isProduction) return [3 /*break*/, 4];
                                            return [4 /*yield*/, run_webpack_1.runWebpackAzureFunction(functionDirsOrFiles_1, shouldInject)];
                                        case 3:
                                            _a.sent();
                                            _a.label = 4;
                                        case 4:
...
if (process.argv.filter(function (x) { return x === '-w'; }).length > 0) {
    watch(["./" + srcPathName, './resources', './package.json'], function () { return runWhenReady(); });
}
